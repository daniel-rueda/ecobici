...
{
	for (DRStation *poi in [placesOfInterest objectEnumerator]) {
...
- (void)updatePlacesOfInterestCoordinates
{
	...
	for (DRStation *poi in [[self placesOfInterest] objectEnumerator]) {
		double poiX, poiY, poiZ, e, n, u;
		
		latLonToEcef(poi.coordinate.latitude, poi.coordinate.longitude, 0.0, &poiX, &poiY, &poiZ);
	...
	for (NSData *d in [orderedDistances reverseObjectEnumerator]) {
		...
		DRStation *poi = (DRStation *)[placesOfInterest objectAtIndex:distanceAndIndex->index];
